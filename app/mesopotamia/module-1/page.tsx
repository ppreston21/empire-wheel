...
export default function ModuleOnePage() {
  return <main className="relative mx-auto max-w-6xl px-6 py-14 lg:px-10 lg:py-20">
    ...
    <div className="mt-12 grid gap-12 lg:grid-cols-[1fr_220px]"><article className="space-y-14"><ModuleOneLesson /></article><aside className="order-first lg:order-last"><nav aria-label="Module sections" className="border-t border-white/10 pt-5 lg:sticky lg:top-8"><p className="text-xs font-bold uppercase tracking-[.18em] text-stone-500">In this module</p><ol className="mt-5 grid grid-cols-2 gap-3 text-sm text-stone-500 lg:grid-cols-1">{sections.map((section, index) => <li key={section.id}><a href={`#${section.id}`} className="hover:text-[#d4b77c]"><span className="mr-3 font-serif text-[#8c744a]">{index + 1}</span>{section.label}</a></li>)}</ol></nav></aside></div>
  </main>;
}
